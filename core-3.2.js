import { MonotonicClock, Clock } from './util-3.2.js';
import { Scheduler } from './util-3.2.js';
import { PsychObject } from './util-3.2.js';
import { toString, getUrlParameters, to_win } from './util-3.2.js';
import { ExperimentHandler } from './data-3.2.js';
import { Color } from './util-3.2.js';
import { Logger } from './util-3.2.js';
...
class Window extends PsychObject {
		...
		if (this.fullscr) {
			...
			if (typeof document.documentElement.requestFullscreen === 'function')
				document.documentElement.requestFullscreen()
					.catch( () => { this.psychoJS.logger.warn('Unable to go fullscreen.'); });
	...
	closeFullScreen() {
		if (this.fullscr) {
			this._psychoJS.logger.debug('Resizing Window: ', this._name, 'back from fullscreen.');
			if (typeof document.exitFullscreen === 'function')
				document.exitFullscreen()
					.catch( () => { this.psychoJS.logger.warn('Unable to go back from fullscreen.'); });
	...
	render() {
		...
		if (typeof this._renderer.gl !== 'undefined') {
			if (typeof this._renderer.gl.readPixels !== 'undefined')
...
class GUI
{
	...
	dialog({
		...
		if (typeof error !== 'undefined')
		{
			...
			if (errorCode)
			{
				htmlCode += this._getUserFriendlyErrorMessage(errorCode);
			}
			else
			{
	...
	_getUserFriendlyErrorMessage(errorCode)
	{
		switch (errorCode)
		{
			case 1:
				return '<p>Oops we encountered an internal server error.</p><p>Try to run the experiment again. If the error persists, contact the experiment designer.</p>';
			case 21:
				return '<p>The experiment is currently inactive and cannot be run.</p><p>If you are the experiment designer, go to your <a href="https://pavlovia.org/dashboard">dashboard</a> and change the experiment status to either PILOTING or RUNNING.</p>';
			default:
				return '<p>Unfortunately we encountered an unspecified error.</p><p>Try to run the experiment again. If the error persists, contact the experiment designer.</p>';
		}
	}
}
...
export { BuilderKeyResponse, EventManager, GUI, KeyPress, Keyboard, MinimalStim, Mouse, PsychoJS, ServerManager, Window, WindowMixin };
